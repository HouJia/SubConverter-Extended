...
- [11. 与 HouJia/SubConverter-Extended 仓库的关系](#11-与-houjiasubconverter-extended-仓库的关系)
...
## 11. 与 HouJia/SubConverter-Extended 仓库的关系

| 仓库 | 角色 |
|------|------|
| **[HouJia/SubConverter-Extended](../SubConverter-Extended)** | Extended **官方 fork 镜像**；用于 `git fetch upstream` 对齐 Aethersailor，**不是**当前 NAS 交付仓库 |
| **`HouJia/subconverter`（本仓库）** | **实际交付主线**：Extended 代码基线 + tindy/MetaCubeX diff 补缺 + NAS/`houjia` 自建层 |

**为何代码在 subconverter 而不只在 SubConverter-Extended fork？**

1. 本仓库 historically fork 自 tindy，已建立 **`upstream-subconverter` / `upstream-subconverter-MetaCubeX` 镜像分支**，三路 diff 补缺流程在此运行更方便。  
2. Extended 是 **整棵替换基线**，不是「在 tindy 上小改」；交付物（`deploy/nas/*`、houjia 模板、版本 `+houjia.N`）与 Extended 官方 release 节奏不同，放在集成仓库更清晰。  
3. `HouJia/SubConverter-Extended` 仍建议 **跟踪 Aethersailor/master**，便于对比 Extended 更新；合入时以 **本仓库 `hjsmaster`** 为准。

**若希望「语义上更干净」**：长期可将 **`HouJia/SubConverter-Extended` 的 `houjia` 分支** 设为 Extended + 自建补丁的主开发仓，本仓库仅保留上游镜像与文档——需一次性迁移 remote/CI/NAS 脚本，**当前未执行**，避免双仓重复维护。
